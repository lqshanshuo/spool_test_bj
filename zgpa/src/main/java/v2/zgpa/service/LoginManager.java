...
import v2.zgpa.facade.LoginFacade;
...
//    private static final String QueryUrl = "http://localhost:9118/service_generic_query/api/query/native";
    ...
    public ResponsePOJO login(String personal_code, String passwd, String staff_type, String key) {
        ...
        try {
            ...
            if ("YINGXIAO".equals(staff_type)) {
                ...
                yingxiaostaffentity = loginFacade.loginYingxiao(type, personal_code);
            } else if ("QUTUO".equals(staff_type)) {
                class_name = "Qutuostaffentity";
                type = "QUTUO_STAFF_INFO";
                qutuostaffentity = loginFacade.loginQutuo(type, personal_code);
                System.out.println("type = " + type);
            }

            
            if (qutuostaffentity == null && yingxiaostaffentity == null) {
                ...
                String id_number;
                if ("YINGXIAO".equals(staff_type)) {
                    id_number = yingxiaostaffentity.getId_last_four();
                    user_message = JsonUtil.toJson(yingxiaostaffentity);
                } else {
                    id_number = qutuostaffentity.getBirthday().substring(4, 8).replaceFirst("^0+", "");
                    user_message = JsonUtil.toJson(qutuostaffentity);
                }
